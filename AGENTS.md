...
- World: 1,800 metres, one unit per metre, layout version 8, six authored biome/sky/ecology families, 48 infrastructure sites, six walkable ascent structures, six forts, a keep, and connected settlement POIs on one fixed continent. Primitive procedural forest and scattered tree-prop layers are disabled. Preserve the owner-supplied three-LOD fixed forest in Drowned Coast, Verdant Ruins, and Sky-Sunder Peaks; Frostbound replaces it with the deterministic green-Christmas/yellow-winter three-LOD blend under `assets/models/frostbound-trees/`. Ember Dunes and Moonfall counts must remain zero. Preserve authored/imported/Forge tree placements independently.
- Sky: Drowned Coast, Ember Dunes, Frostbound Wilds, Sky-Sunder Peaks, and Moonfall use owner-supplied local equirectangular panoramas with procedural fallbacks; Verdant Ruins remains procedurally generated.
- Water: every registered finite water mesh uses the attributed Cortiz-inspired cel/Voronoi material factory. The current authored surface is the bounded Drowned Coast disc; preserve its waterline, shallow traversal, deep-water rejection, save restoration, mobile tier, and reduced-motion behavior.
...
- Combat: blade, bow, axe, staff; animation-timed release, dodge/i-frames, target lock, telegraphs, hit-stop, shake, knockback, dragons, and tameable creatures. Preserve the owner-supplied measured weapon models and projectile visuals with their procedural load-failure fallbacks.
- Conquest: no wave mode remains. Every generated location is a stronghold with a level-scaled garrison. Forts/keep can gain golems at higher levels. Clearing grants a kind-specific bonus. Preserve owner-supplied hostile banners at forts/keep and the blue flag/emblem raised at captured shrines/graveyards without moving capture authority into presentation state.
