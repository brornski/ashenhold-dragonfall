...
World controls edit the one authored continent rather than generating a new seed. Each biome can override ground, cliff, grass, and fog colors; fog density and exposure; and prop and grass density. Changes preview live. Density values above currently allocated instance capacity take full effect after **Save Repo** and reload. The procedural forest and scattered procedural tree layers are currently disabled across all biomes so trees can be rebuilt deliberately; authored/imported POI trees and Forge placements remain. Drowned Coast, Ember Dunes, Frostbound Wilds, Sky-Sunder Peaks, and Moonfall use supplied equirectangular panoramas under `assets/textures/skyboxes/`; each biome retains its generated sky as a load-failure fallback.
...
Procedural ancient forests, hero-tree generation, their instanced near/far meshes, and their trunk colliders are disabled across all six biomes. The scattered procedural snow pines, broadleaf trees, palms, and dark pines are also skipped. Tree assets remain in the model catalog for deliberate POI, Forge, and future authored placement. Seeded infrastructure still adds 24-32 biome-specific micro-landmarks such as collapsed walls, abandoned farms, hunter platforms, drowned piers, carts, and waystones outside protected routes and strongholds.
