...
Forge is freecam-first and opens in Freecam. Scene selection, gizmo transforms, and inspector editing all remain available without leaving it. Use `F` for Freecam, `V` for Select, and `N` for Noclip. The backtick key collapses or restores the Forge panel. `Ctrl/Cmd+Z` undoes, `Shift+Ctrl/Cmd+Z` or `Ctrl/Cmd+Y` redoes, and Delete/Backspace removes the selected entity. The browser draft keeps up to 60 editor history states.

Select an entity with a short left click on its world model or choose it from the searchable, category-filtered Scene hierarchy. Selection renders a bounds box and axis gizmo. `G`, `R`, and `X` select move, rotate, and scale tools; left-dragging a gizmo applies the active transform, while the inspector accepts exact XYZ position, Y rotation, and XYZ scale. Gizmo dragging and inspector edits honor configurable move, rotation, and scale snapping (defaults: 1 metre, 15 degrees, and 0.1 scale). Selected objects can be framed, duplicated, collision-enabled/disabled, dropped to terrain, or reset. Decorative and custom models can also be hidden/shown or removed. Forge deliberately locks Hide and Remove for chests, enemies, dragons, and location groups so authoritative interaction, AI, stronghold, and progression state can never remain active behind an invisible object.
...
Freecam moves independently of the Warden: `WASD` moves relative to the view, `Q`/`E` descends/ascends, Shift applies a 3.2x boost, and the wheel adjusts camera speed. A short left click selects; left-dragging away from a gizmo after the movement threshold or right-dragging looks. Pressing `V` from freecam returns the Warden to the camera's exact XYZ and facing, clears stale movement, combat, and pose state, and enters Select. Pressing `N` from freecam places the Warden at that position and starts noclip, which moves the Warden through world collision with the same movement/elevation inputs and Shift boost. The simulation toggle lets AI and effects run without leaving Forge.
