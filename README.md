...
Ashenhold is now one permanent, authored 1,800-metre continent instead of a generated realm selected by biome and seed. The Drowned Coast, Verdant Ruins, Ember Dunes, Frostbound Wilds, Sky-Sunder Peaks, and Moonfall Expanse occupy fixed geographic zones in the same live scene. Walking across a border changes terrain materials, fog, lighting, skybox, vegetation, props, enemy roster, and location presentation without a reload. Procedural forests and scattered procedural tree props are currently removed from every biome so the tree population can be rebuilt deliberately; authored/imported POI trees and Forge placements remain. Regional PBR and creature assets stream ahead near biome boundaries, keeping distant defenders hydrated in place. Fixed forts, ascents, settlements, shrines, ruins, camps, and infrastructure keep the layout consistent for exploration, saves, and co-op parties.
...
The world now uses a measured one-unit-per-metre model registry and layout version 7. Settlement houses, castle walls, gates, towers, colliders, and door clearances share player-readable dimensions instead of pack-specific source scales. Each biome also receives its own sky and horizon treatment plus 24-32 seeded infrastructure micro-landmarks. The former ancient instanced forest system remains available in source but is disabled while the tree layout is rebuilt. Captured shrines and graveyards raise persistent Warden flags and add a matching minimap marker.
...
The FreeStylized hero-tree derivatives `assets/models/freestylized-foliage/F1_Tree1.glb` and `F1_Tree3.glb` remain available in the model catalog for deliberate placement, but are no longer procedurally instantiated. They were converted, welded, and quantized for the browser; the source pack's large texture set is intentionally omitted so the meshes receive Ashenhold's biome palette at runtime. Exact source links, adaptations, and redistribution terms are recorded in [ATTRIBUTIONS.md](ATTRIBUTIONS.md) and the notices beside each asset family.
