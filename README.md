...
Ashenhold is now one permanent, authored 1,800-metre continent instead of a generated realm selected by biome and seed. The Drowned Coast, Verdant Ruins, Ember Dunes, Frostbound Wilds, Sky-Sunder Peaks, and Moonfall Expanse occupy fixed geographic zones in the same live scene. Walking across a border changes terrain materials, fog, lighting, skybox, vegetation, props, enemy roster, and location presentation without a reload. The former primitive tree generators remain disabled. A supplied three-stage tree model forms the fixed forest in Drowned Coast, Verdant Ruins, and Sky-Sunder Peaks, while Frostbound Wilds replaces that family with a deterministic blend of the supplied green Christmas and yellow winter three-LOD trees. Total density remains 2,200 desktop trees and 990 on coarse devices, with chunk-level LOD and culling; Ember Dunes and Moonfall receive none. Authored/imported POI trees and Forge placements remain independent. Regional PBR and creature assets stream ahead near biome boundaries, keeping distant defenders hydrated in place. Fixed forts, ascents, settlements, shrines, ruins, camps, and infrastructure keep the layout consistent for exploration, saves, and co-op parties.

Captured shrines and the Moonfall graveyard now raise a 20-22 metre animated Warden standard that persists in the active save and appears on the minimap. The standard uses the supplied blue/white flag and emblem models, while uncaptured forts and Ashenhold Keep display supplied red/black hostile banners. The Warden's blade, bow, axe, staff, arrows, and thrown axes also use the supplied medieval weapon models and compact atlases, with measured metre-scale placement and procedural load-failure fallbacks. Normal sprint and super-sprint jumps retain horizontal takeoff velocity through rise and fall, with limited air steering and a dedicated airborne pose. The local player and remote party members use the same supplied Hooded Shadow Assassin Warden model, with server-stable model, marker, and nameplate colors distinguishing each connected Warden. Its procedural run cycle deforms the otherwise unrigged model into alternating arm, thigh, knee, and cloak motion, scaling stride intensity for running, sprinting, and super sprinting while blending smoothly back to idle.
...
Drowned Coast, Ember Dunes, Frostbound Wilds, Sky-Sunder Peaks, and Moonfall now use the project owner's supplied equirectangular panoramas, isolated to their respective regions. Each prior procedural sky remains an automatic load-failure fallback.
...
The world now uses a measured one-unit-per-metre model registry and layout version 8. Settlement houses, castle walls, gates, towers, colliders, and door clearances share player-readable dimensions instead of pack-specific source scales. Each biome also receives its own sky and horizon treatment plus 24-32 fixed infrastructure micro-landmarks. The supplied tree forest uses the measured model registry, biome-specific height/tint profiles, route and POI clearance, three distance LODs, and a bounded set of giant-trunk colliders. Captured shrines and graveyards raise persistent Warden flags and add a matching minimap marker.
...
The FreeStylized hero-tree derivatives `assets/models/freestylized-foliage/F1_Tree1.glb` and `F1_Tree3.glb` remain available in the model catalog for deliberate placement, but are no longer procedurally instantiated. The active fixed forest uses the owner-supplied `assets/models/tree-lods/` LOD0/LOD1/LOD2 set with shared compact color textures. Exact source links, adaptations, provenance, and redistribution terms are recorded in [ATTRIBUTIONS.md](ATTRIBUTIONS.md) and the notices beside each asset family.
